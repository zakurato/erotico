...
<head>
    ...
    <title>Crear categoría sin color ni tamaño</title>
...
<body>
    ...
    <div class="container">
        <h2>Crear categoría sin color ni tamaño</h2>
        <br><br>
        ...
        <br><br>
        <form action="{{ 'storeCategoriaSinColorNiTamaño' }}">
            @csrf
            <div class="form-group">
                <label>Nombre de la categoría sin color ni tamaño:</label>
                <input type="text" class="form-control" name="SinColorNiTamaño" autocomplete="off"
    ...
    <br><br>
    <h2>Tabla de categorías sin color ni tamaño</h2>
    <br>
    <table class="table">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($categorias as $item)
                <tr>
                    <td>{{ $item->nombreCategoriaSinColorNiTamaño }}</td>
                    <td>
                        <form id="actualizarForm" action="{{ route('actualizarCategoriaSinColorNiTamaño') }}" method="GET">
                            @csrf
                    ...
                    <td>
                        <form id="eliminarForm-{{ $item->id }}" action="{{ route('eliminarCategoriaSinColorNiTamaño') }}" method="POST">
                            @csrf
                            <input type="text" name="id" value="{{ $item->id }}" hidden>
                            <button type="button" class="bntEliminarCategoria" onclick="confirmarEliminacion('{{ $item->nombreCategoriaSinColorNiTamaño }}', '{{ $item->id }}')">
